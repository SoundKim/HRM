...
namespace HRM.WebMVCApp.Controllers
{
    public class EmployeeRoleController : Controller
    {
        ...
        {
            var employeeRoleCollection = await employeeRoleServiceAsync.GetAllEmployeeRolesAsync();
        ...
        public async Task<IActionResult> Delete(EmployeeRoleRequestModel model)
        {
            try
            {
                var result = await employeeRoleServiceAsync.DeleteEmployeeRoleAsync(model.Id);
                return RedirectToAction("Index");
            }
            catch (Exception ex)
            {
